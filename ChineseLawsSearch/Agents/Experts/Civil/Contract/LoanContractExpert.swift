...
let loanContractExpert = SubExpert(
    name: "借款合同专家",
    domain: "民间借贷、银行贷款、利息约定、还款期限、逾期利息、借条、保证担保、抵押担保",
    requiredInfo: [
    ...
    chapterIdHints: chLoan + chGuarantee + chJiuMinGuarantee,
    ftsDomains: ["民法典"],
    ftsCategories: ["法律", "司法解释"],
    ftsKeywordsExtra: ["民间借贷", "利率上限", "逾期利息", "保证期间", "一般保证", "连带保证",
                       "超过诉讼时效", "催款通知", "越权担保", "善意相对人", "担保合同无效",
                       "抵押权", "质押权", "执行担保"],
    answerTemplate: """
    ...
    3. 逾期还款的利息计算规则
    4. 担保效力分析（参考九民纪要第17-20条）：
       - 公司越权担保的善意认定（是否审查股东会/董事会决议）
       - 一般保证vs连带责任保证，保证期间（未约定时6个月）
       - 担保合同无效后的民事责任（九民纪要第20条）
