    ...
    var body: some View {
        ScrollViewReader { proxy in
            ...
            .task(id: "\(target.law.id)-\(target.scrollToArticle ?? 0)-\(userStore.lawsExamMode)") {
                ...
                // Set nodes first, then imperatively scroll while overlay is still covering.
                nodes = loadedNodes
                await Task.yield()
                // Imperative scroll forces LazyVStack to realize the target cell even if off-screen.
                if let tid = targetNodeId {
                    proxy.scrollTo(tid, anchor: .top)
                } else if let firstId = loadedNodes.first?.id {
            ...
            .onChange(of: target.scrollToArticle) { _, artNum in
                ...
                if let targetNode = nodes.first(where: { $0.articleNum == artNum }) {
                    ...
                    proxy.scrollTo(targetNode.id, anchor: .top)
                    withAnimation(.easeIn(duration: 0.2)) { highlightedArticle = artNum }
                    highlightTask?.cancel()
                    highlightTask = Task {
                        try? await Task.sleep(for: .seconds(1.5))
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .navigationBarTrailing) {
                    HStack(spacing: 16) {
                        if canGoBack || showMenuButton {
                            Button { goToMenu() } label: {
