...
final class LegalExpertService {
    ...
    private func analyzeWithExpertMode(expert: SubExpert, question: String,
        ...
        【引用条文扩展要求】当某条款引用了另一条文（如"依照本法第X条"、"参照适用"等），必须同时分析被引用条文的要件是否满足，说明其对结论的影响，不得只引用"入口"条文而忽略其指向的实体规定。
        严禁使用Markdown格式。
        """
        do {
    ...
    private func characterizeAndRoute(question: String,
        ...
        - 涉及公司股权/担保/决议→公司商事专家
        - 刑事实体法问题（罪名认定/构成要件/量刑/刑法条文）→按犯罪类型选刑法专家（财产犯罪/人身伤害/经济犯罪/腐败职务犯罪）；交通肇事罪/危险驾驶罪→人身伤害专家
        - 刑事诉讼专家仅用于刑事程序问题（管辖/侦查/起诉/辩护/羁押），不用于罪名认定
        - 有程序问题→对应程序专家（民事/刑事/行政）
        - 不要因关键词相似选入实际无关的专家（如"交通事故"涉及刑事罪名时不选交通事故责任专家）
        """
...
【引用条文扩展要求】法条中常存在对其他条文的引用（如"依照本法第X条"、"适用本编/本章规定"、"参照适用"等）。当你分析某条款时，如果该条款本身或其适用效果指向另一条文，必须：
（一）明确指出被引用条文的编号和核心内容；
（二）判断被引用条文的要件是否也满足用户案情；
（三）说明被引用条文对结论的影响（是扩大责任范围、限制适用条件，还是提供例外情形）。
不得只引用"入口"条文而忽略其指向的实体规定。例如：某条规定"参照本法第XX条处理"，则第XX条的要件和法律效果同样需要对照用户案情分析。
...
（二）用户问题属于检索性或概括性（如"帮我列出交通事故相关的指导案例"、"有哪些关于XXX的判例"），此时可直接列举相关案例，无需强行比对事实。
禁止因为案例涉及同一法律领域就引用——仅领域相同、事实不相似的，不得引用。
引用格式：《案例标题》确立的裁判规则……（须结合案情说明关联，不得只列案例名称）。
不得捏造或扩大解释案例内容。
...
"""
    ...
    要求：去除重复内容，保留最重要的结论；突出条文引用（保留《XXX》第X条格式）；内容完整，不因字数截断。
    ...
    - 【公报案例引用规则】如果上下文提供了"人民法院公报候选案例"，仅在以下两种情形引用：
      （一）用户有具体案情，且候选案例与用户案情存在相似的法律关系和核心事实（当事人类型、行为性质、争议焦点高度吻合）——此时可引用，须说明事实相似之处；
      （二）用户问题属于检索性/概括性（如"列举XX相关指导案例"），此时可直接列举，无需强行比对事实。
      仅领域相同但事实不相似的案例，不得引用。
      引用格式（每个案例单独一行，格式固定）：
      【参考案例】《案例完整标题》：说明该案裁判规则及与本案的关联。
    格式要求：
    ...
    5b.【条件式分析结构】当某法律问题的结论取决于事实认定（如"是否构成劳动关系""是否存在过错"）时，必须按以下顺序写：
      ① 先列出法律/司法解释/指导案例确立的认定标准（"认定XX需满足以下条件：……"），
      ② 再对照案情逐条分析哪些满足、哪些不明确，
      ③ 最后给出条件式结论（"若…成立则…；若…不成立则…"）。
      禁止先写"现有证据不足以认定"再在后文案例部分补充认定标准——认定标准必须在结论之前出现，不能倒置。
    6. 如涉及诉讼，注明应去哪个法院。
