...
**订阅状态显示修复**
- 修复订阅成功后，设置页、咨询页仍显示"免费剩余次数"而非"已订阅"的问题（根因：免费次数尚有剩余时 `access` 返回 `.free`，导致订阅状态被覆盖）
- 修复订阅用户点击高院公报详情仍弹出付费墙的问题（`canViewGazetteDetail` 现在直接判断 `hasPRO`，不再依赖 `access`）

**订阅次数消耗逻辑修复**
- 修复订阅后仍优先消耗免费次数的问题；现在订阅激活后优先消耗订阅配额，免费次数冻结保留，取消订阅后可继续使用

**历史记录闪跳修复**
- 修复历史记录列表打开时频繁自动刷新闪跳的问题；iCloud 同步事件的 debounce 从 0.5 秒延长至 2 秒，且 load 完成后仅在内容实际变化时才更新列表
